...
class PostService extends BaseService
{
    ...
    public function addPostByVin($post, $userID)
    {
        ...
        if (!isset($post['body'])) {
            throw new Exception("Post body is missing!!!");
        }
        if (!isset($car['id']) || $car['user_id'] != $userID) {
            throw new Exception("This car does not exist or belong to you!", 401);
        }
    ...
    {
        if (!isset($post['body'])) {
            throw new Exception("Body of the post is missing!");
        }
