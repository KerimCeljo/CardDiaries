...
class UserService extends BaseService
{
    ...
    private DOSpacesClient $DOSpacesClient;
    ...
    public function reset($user)
    {
        ...
        if (strtotime(date(Config::DATE_FORMAT)) - strtotime($dbUser['token_created_at']) > 300) {
    ...
    public function forgot($user)
    {
        ...
        if (strtotime(date(Config::DATE_FORMAT)) - strtotime($dbUser['token_created_at']) < 300) {
            throw new Exception("Be patient, token is on its way!", 400);
        }
    ...
    public function login($user)
    {
        ...
        if ($dbUser['status'] != 'ACTIVE') {
            throw new Exception("Your account is not active!", 400);
        }
